...
date: 2017-07-24
categories: ["R"]
tags: ["Golf", "Word cloud", "Visulation"]
...
3. Combine words that are the same but not identical becuase of spelling (e.g. dog leg, dog-leg, dogleg)  
4. Create wordclouds of different lengths to account not only for words but phrases  
5. Predict?  

##Scraping

I am going to create a function to return text hole data from a specific tournament and year. The year will not matter for our case of the **Players Championship** but it will matter for other touranments such as the **Open** or **US Open**.
...
## Finally a Wordcloud
Now we can actaully create our wordcloud to have a visual of word frequencies for the entire course.
...
## N-Grams
**N-Grams** are a contiguous sequence of **n** items of a given text. For example, instead of just the words "**dogleg**" and "**left**" a 2-gram or bigram would be "**dogleg left**". A trigram would be something like "**left fairway bunker**".  
...
However, from looking at the above bigram we can make further conclusions about the course. For example, we find new phrases such as "**short iron**" and "**fairway wood**". These can give indications that on many holes these clubs will be used so looking at approaches from distances of 100 - 150 for **short irons** and approaches from distances of 250 -275 for **fairway wood** will help find players that fit this course.  

Now lets see if trigrams will be able to provide any more insight.  
```{r}
...
Unfortunately, from this trigram not much can be drawn other than maybe we have some par 5s that can be reached in two.  

##Takeaway
Going through these word clouds we can make a few assements. We have learned that this course has **water** as **hazards** and that the **fairway wood** and **short irons** will be important. We also can assume that distance will be important since the golfers can get on the par 5s **green in two shots**.  

While it is typicall to go through hole by hole and figure out which shots will be more important and how many par 5s are reachable in two for which golers, this give a good visual overview of the course. This would be a great place to start analysis of a course and a good simple way to compare courses against each other by looking at the frequencies of words used.  
